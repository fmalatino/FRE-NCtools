...
void setup_aligned_nest(int parent_ni, int parent_nj, const double *parent_xc, const double *parent_yc,
                        ...
                        double *xc, double *yc, int is_gr, int parent_tile);

void setup_aligned_nest2(int parent_ni, int parent_nj, const double *parent_xc, const double *parent_yc,
                        int halo, int refine_ratio, int istart, int iend, int jstart, int jend,
                        double *xc, double *yc, int is_gr, int parent_tile, int *tile_offset);
  ...
  if(global_nest) {
    ...
    for(n=0; n<ntiles; n++) {
      ...
      setup_aligned_nest(ni2, ni2, xc2+ni2p*ni2p*n, yc2+ni2p*ni2p*n, 0, refine_ratio[0],
                         1, ni2, 1, ni2, xc+n*nip*nip, yc+n*nip*nip, 1, parent_tile[0]);
    }
  }
    ...
    for (nn=0; nn < num_nest_grids; nn++) {
      ...
      //                    xc+tile_offset[ntiles+nn], yc+tile_offset[ntiles+nn], 0, parent_tile[nn]);
      setup_aligned_nest2(ni_parent[nn], nj_parent[nn], xc,
                         yc, halo, refine_ratio[nn],
                         istart[nn], iend[nn], jstart[nn], jend[nn],
                         xc+tile_offset[ntiles+nn], yc+tile_offset[ntiles+nn], 0, parent_tile[nn], tile_offset);
    }
  ...
  if(global_nest) {
    ...
    for(n=0; n<ntiles; n++) {
      ...
      setup_aligned_nest(ni2, ni2,xc2+offset_p, yc2+offset_p, 0, refine_ratio,
                            1, ni2, 1, ni2, xc+n*nip*nip, yc+n*nip*nip, 1, parent_tile);
    }
  }
  else{
...
int index_an_gr2(int jcf, int p_npi, int icf, int max_ni, int max_nj, int parent_tile, int *tile_offset){
  // if(jcf > max_nj){
  //   jcf = max_nj;//then use the uppr (last) row data
  //   //fprintf(stderr, "[INFO] index_an_gr : jcf=%d max_nj=%d\n",jcf, max_nj);
  //   // if(is_gr == 0){          
  ...
  //   }
  // }
  // fprintf(stderr, "jcf = %d, p_npi = %d, icf = %d, max_ni = %d, max_nj = %d, out = %d\n", jcf, p_npi, icf, max_ni, max_nj, jcf * p_npi + icf);

  int tn, ts, te, tw, index, overlap=0;
  if(parent_tile%2==0)
  {
    tn = parent_tile + 1;
    te = parent_tile + 2;
  ...
  else
  {
    ...
    return tile_offset[parent_tile - 1] + jcf * p_npi + icf;
  }
                        ...
                        double *xc, double *yc, int is_gr, int parent_tile, int *tile_offset)
{
  ...
  /*ERASE ME*/
  fprintf(stderr, "\n\nni = %d, nj = %d, npi = %d, npj = %d, parent_npi = %d\n", ni, nj, npi, npj, parent_npi);
  ...
  for(j=0; j<npj; j++) {
    ...
    for(i=0; i<npi; i++) {
      ...
      imod = i%refine_ratio;
       if(jmod == 0) {
        int idx = index_an_gr2(jc,parent_npi,ic,parent_ni,parent_nj,parent_tile, tile_offset);
        int idx_pi = index_an_gr2(jc,parent_npi,ic+1,parent_ni,parent_nj,parent_tile, tile_offset);
        index = idx;
        q1[0] = parent_xc[ idx ];
        q1[1] = parent_yc[ idx ];
        q2[0] = parent_xc[ idx_pi ];
        ...
        int idx = index_an_gr2(jc,parent_npi,ic,parent_ni,parent_nj,parent_tile, tile_offset);
        int idx_pi = index_an_gr2(jc,parent_npi,ic+1,parent_ni,parent_nj,parent_tile, tile_offset);
        int idx_pj = index_an_gr2(jc+1,parent_npi,ic,parent_ni,parent_nj,parent_tile, tile_offset);
        int idx_pjpi = index_an_gr2(jc+1,parent_npi,ic+1,parent_ni,parent_nj,parent_tile, tile_offset);
        index = idx;
        t1[0] = parent_xc[ idx ];
        t1[1] = parent_yc[ idx ];
        t2[0] = parent_xc[ idx_pj ];
